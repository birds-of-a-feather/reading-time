...
function delete_project () {
  curl -H "Authorization: Token $GITHUB_TOKEN" -H "Accept: $ACCEPT_HEADER" -H "Content-type: application/json" -X DELETE https://api.github.com/projects/$1
}
...
if [[ $REPLY =~ ^[Yy]$ ]]
then
    for project_id in $(curl -s -H "Authorization: Token $GITHUB_TOKEN" -H "Accept: $ACCEPT_HEADER" "https://api.github.com/repos/$RT_ORG/$RT_REPO/projects" | jq -r '.[].id')
    do
...
PROJECT_ID=`curl -s -H "Authorization: Token $GITHUB_TOKEN" -H "Accept: $ACCEPT_HEADER" -H "Content-type: application/json" -X POST -d @$DIR/projects/project1.json https://api.github.com/repos/$RT_ORG/$RT_REPO/projects | jq .id`
