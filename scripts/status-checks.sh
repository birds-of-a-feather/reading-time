...
function statuses () {
  curl -H "Authorization: Token $TOKEN" -H "Accept: application/json" -H "Content-type: application/json" -X POST -d '{"state": "'"$1"'","target_url": "https://travisci.octodemoapps.com/'$TRAVIS_REPO_SLUG'","description": "'"$2"'","context": "'$3'-status-check/travis-ci"}' https://api.github.com/repos/${TRAVIS_REPO_SLUG}/statuses/$TRAVIS_COMMIT
}
